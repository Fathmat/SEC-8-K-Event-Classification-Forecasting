...
def extract_data_from_report(report_text, filename):
    ...
    for I_pattern in item_pattern:
        match = re.search(I_pattern, report_text)
        if match:
            item_numbers = match.group(0)

    # Preprocess text and categorize document
    preprocessed_text = preprocess(report_text)
    return company_name, report_date, item_numbers
                    ...
                    if int(vr) in cik_list:
                        if filename.endswith('.txt') and '(8-k)' in filename.lower():
                            with open(full_path, 'r', encoding='utf-8') as file:
                                ...
                                company_name, report_date, item_numbers = extract_data_from_report(report_text, filename)
...
output.to_excel('extracted_data539.xlsx', index=False)
